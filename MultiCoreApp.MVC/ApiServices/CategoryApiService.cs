...
namespace MultiCoreApp.MVC.ApiServices
{
    public class CategoryApiService
    {
        ...
        public async Task<IEnumerable<CategoryDto>> GetAllAsync()
        {
            ...
            {
                categoryDtos =
                    JsonConvert.DeserializeObject<IEnumerable<CategoryDto>>(
        ...
        public async Task<bool> Edit(CategoryDto catDto)
        {
            var stringContent =
                new StringContent(JsonConvert.SerializeObject(catDto), Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"category/{catDto.Id}", stringContent);
            if (response.IsSuccessStatusCode)
            {
                return true;
            }
